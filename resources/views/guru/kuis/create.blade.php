    ...
    <form method="POST" action="{{ route('guru.kuis.store') }}" enctype="multipart/form-data" @submit="validateBeforeSubmit">
        ...
        <x-card title="Informasi Kuis" class="mb-6">
            ...
            <div class="mb-6">
                ...
                <select name="modul_iqra_id" required
                    ...
                    @foreach($modules as $module)
                        <option value="{{ $module->id }}" {{ (isset($modulIqraId) ? $modulIqraId : '') == $module->id ? 'selected' : '' }}>
                            {{ $module->nama_modul }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="judul_kuis" required value="{{ old('judul_kuis') }}"
                       class="w-full px-4 py-3 text-lg border-2 border-gray-300 rounded-lg focus:border-emerald-500 focus:ring focus:ring-emerald-200"
        ...
        <x-card class="mb-6">
            ...
            <div x-show="pertanyaan.length === 0" class="text-center py-12">
                <x-icon name="file" class="w-16 h-16 mx-auto mb-4 text-gray-300" />
                <p class="text-lg text-gray-500">Belum ada pertanyaan</p>
                <button type="button" @click="addQuestion" 
                        class="inline-flex items-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition mt-4">
