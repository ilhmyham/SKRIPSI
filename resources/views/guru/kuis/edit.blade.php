...
<div class="max-w-6xl mx-auto" x-data="quizBuilder()">
    <form method="POST" action="{{ route('guru.kuis.update', $kuis) }}" enctype="multipart/form-data" @submit.prevent="submitProcess">
        @csrf
        @method('PUT')
        ...
        <x-card title="Informasi Kuis" class="mb-6">
            ...
            <div class="mb-6">
                ...
                <select name="modul_iqra_id" id="modul_iqra_id" required
                    ...
                    @foreach($modules as $module)
                        <option value="{{ $module->id }}" {{ $kuis->modul_iqra_id == $module->id ? 'selected' : '' }}>
                            {{ $module->nama_modul }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="judul_kuis" id="judul_kuis" required value="{{ old('judul_kuis', $kuis->judul_kuis) }}"
                       class="w-full px-4 py-3 text-lg border-2 border-gray-300 rounded-lg focus:border-emerald-500 focus:ring focus:ring-emerald-200">
            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div x-show="question.gambar_preview" class="mt-2">
                                <div class="relative inline-block">
                                    ...
                                    <button type="button" aria-label="Hapus Gambar Pertanyaan Baru"
                                ...
                                <template x-for="(opsi, oIndex) in question.opsi" :key="oIndex">
                                    <div class="p-3 bg-white rounded-lg border-2 border-gray-200">
                                        <div class="flex items-start gap-3 mb-2">
                                            ...
                                            <!-- Option Text -->
                                            <div class="flex-1">
                                                <label :for="'edit-o-text-' + qIndex + '-' + oIndex" class="sr-only">Teks Opsi</label>
                                                <input type="text" 
                                                       :id="'edit-o-text-' + qIndex + '-' + oIndex"
                                        ...
                                        <div class="ml-8">
                                            ...
                                            <input type="file" 
                                                   ...
                                                   class="hidden"
                                                   :id="'edit-o-img-' + qIndex + '-' + oIndex">
                                            
                                            <div x-show="!opsi.gambar_preview">
                                                <label :for="'edit-o-img-' + qIndex + '-' + oIndex" 
                                            ...
                                            <div x-show="opsi.gambar_preview" class="mt-2">
                                                <div class="relative inline-block">
                                                    ...
                                                    <button type="button" aria-label="Hapus Gambar Opsi Jawaban Baru"
    ...
    <span x-text="pertanyaan.length === 0 ? 'Minimal 1 Pertanyaan' : 'Update Kuis'"></span>
</button>
            <a href="{{ route('guru.kuis.by-module', $kuis->modul_iqra_id) }}" aria-label="Batal dan Kembali ke Daftar Kuis"
    ...
    if ($oldPertanyaan) {
        foreach ($oldPertanyaan as $qIndex => $q) {
            ...
            if (isset($q['opsi'])) {
                foreach ($q['opsi'] as $oIndex => $o) {
                    $opsiList[] = [
                        ...
                        'existing_gambar' => $o['existing_gambar_opsi'] ?? null,
                        'gambar_preview' => null,
            ...
            $initialData[] = [
                ...
                'existing_gambar' => $q['existing_gambar_pertanyaan'] ?? null,
                'gambar_preview' => null,
                'opsi' => $opsiList
            ];
        }
    } else {
        $initialData = isset($quizData) ? $quizData : [];
    }
@endphp
    ...
    return {
        ...
        submitProcess(e) {
            ...
            for (let i = 0; i < this.pertanyaan.length; i++) {
                ...
                // Tangani potensi null dari database menjadi string kosong
                let teksPertanyaan = q.teks_pertanyaan || '';

                // 1. Validasi Pertanyaan Utama 
                if (teksPertanyaan.trim() === '' && !q.gambar_preview && !q.existing_gambar) {
                    alert(`Pertanyaan ke-${i + 1} tidak boleh kosong. Harap isi teks atau biarkan gambar lama/tambah gambar baru.`);
                    isValid = false;
                    break;
                }
                ...
                let validOpsis = q.opsi.filter(o => {
                    // Tangani potensi null dari database pada opsi
                    let teksOpsi = o.teks_opsi || '';
                    return teksOpsi.trim() !== '' || o.gambar_preview !== null || o.existing_gambar !== null;
                });
                ...
                // Timpa data opsi lama dengan data yang sudah bersih
                q.opsi = validOpsis;
