...
@section('content')
    ...
    <div class="mb-6 flex items-center justify-between">
        <nav class="flex items-center gap-2 text-sm text-gray-600">
            <a href="{{ route('guru.kuis.index') }}" class="hover:text-emerald-600">Manajemen Kuis</a>
            <span>/</span>
            <span class="font-semibold">{{ $module->nama_modul }}</span>
        </nav>
        
        <a href="{{ route('guru.kuis.create', ['modul_iqra_id' => $module->id]) }}" aria-label="Tambah Kuis Baru"
           class="inline-flex items-center gap-2 px-6 py-3 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition focus-visible:outline-gray-900">
    ...
    @if($kuisList->isEmpty())
        <x-card>
            <div class="text-center py-12">
                ...
                <a href="{{ route('guru.kuis.create', ['modul_iqra_id' => $module->id]) }}" aria-label="Buat Kuis Pertama untuk {{ $module->nama_modul }}"
                ...
                <x-card :interactive="true">
                    <div class="flex items-start justify-between mb-3">
                        ...
                        <span class="ml-2 px-3 py-1 bg-purple-100 text-purple-700 text-xs font-semibold rounded-full">
                            {{ $kuis->kuis_pertanyaan_count }} soal
                        </span>
                    </div>
                    ...
                    <div class="flex gap-2 pt-3 border-t border-gray-100">
                        <a href="{{ route('guru.kuis.edit', $kuis) }}" aria-label="Edit Kuis {{ $kuis->judul_kuis }}"
                        ...
                        <form method="POST" action="{{ route('guru.kuis.destroy', $kuis) }}" 
