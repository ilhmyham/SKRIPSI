...
$watch('isSidebarOpen', val => {
    ...
    {{-- ── MOBILE OVERLAY ── --}}
    <div x-show="isSidebarOpen && isMobile" @click="closeSidebar()" class="fixed inset-0 z-[55] bg-gray-900/60 lg:hidden"
    ...
    <aside
        ...
        class="bg-gray-900 text-white flex flex-col h-screen top-0 left-0 transition-all duration-300 shrink-0"
        ...
        <div class="p-5 border-b border-gray-700 flex items-center justify-between h-[73px]">
            ...
            <div class="flex items-center gap-3 overflow-hidden"
                :class="(!isSidebarOpen) ? 'justify-center w-full' : 'justify-start'">
                <div class="shrink-0">
        ...
        <!-- Navigation Menu -->
        <nav class="flex-1 px-3 py-5 space-y-1 overflow-y-auto overflow-x-hidden" aria-label="Menu Admin">
            <div x-show="isSidebarOpen || isMobile"
                class="px-3 pb-1 pt-2 text-[10px] font-bold uppercase tracking-widest text-gray-500">Utama</div>

            <a href="{{ route('admin.dashboard') }}" title="Dashboard"
                class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.dashboard') ? 'bg-white text-gray-900' : 'text-gray-300 hover:bg-gray-800' }}"
            ...
            <div x-show="isSidebarOpen || isMobile"
                class="px-3 pb-1 pt-2 text-[10px] font-bold uppercase tracking-widest text-gray-500">Sistem</div>

            <a href="{{ route('admin.users.index') }}" title="Manajemen Pengguna"
                class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.users.*') ? 'bg-white text-gray-900' : 'text-gray-300 hover:bg-gray-800' }}"
                :class="(!isMobile && !isSidebarOpen) && 'justify-center px-0'">
                <x-icon name="users" class="w-5 h-5 shrink-0" />
                <span x-show="isSidebarOpen || isMobile" class="text-sm font-medium whitespace-nowrap">Pengguna</span>
            </a>

            <div class="my-2 border-t border-gray-700/60"></div>
            <div x-show="isSidebarOpen || isMobile"
                class="px-3 pb-1 pt-2 text-[10px] font-bold uppercase tracking-widest text-gray-500">Pendidikan</div>

            <!-- <a href="{{ route('admin.modules.index') }}"
               title="Manajemen Modul"
               class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.modules.*') ? 'bg-white text-gray-900' : 'text-gray-300 hover:bg-gray-800' }}"
               :class="(!isMobile && !isSidebarOpen) && 'justify-center px-0'">
                <x-icon name="book-open" class="w-5 h-5 shrink-0" />
                <span x-show="isSidebarOpen || isMobile" class="text-sm font-medium whitespace-nowrap">Modul Iqra</span>
            </a> -->

            <a href="{{ route('admin.materi.index') }}" title="Manajemen Materi"
                class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.materi.*') ? 'bg-white text-gray-900' : 'text-gray-300 hover:bg-gray-800' }}"
                :class="(!isMobile && !isSidebarOpen) && 'justify-center px-0'">
            ...
            <a href="{{ route('admin.kuis.index') }}" title="Manajemen Kuis"
                class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.kuis.*') ? 'bg-white text-gray-900' : 'text-gray-300 hover:bg-gray-800' }}"
                :class="(!isMobile && !isSidebarOpen) && 'justify-center px-0'">
        ...
        <header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-50">
            <div class="px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-3 min-w-0">
                    <button @click="toggleSidebar()" :aria-expanded="isSidebarOpen.toString()"
                        class="p-2 -ml-1 rounded-lg hover:bg-gray-100 text-gray-500 shrink-0 focus-visible:outline-blue-600">
                ...
                <div class="relative shrink-0">
                    <button @click="profileOpen = !profileOpen" :aria-expanded="profileOpen.toString()"
                        class="flex items-center gap-2 sm:gap-3 px-2 sm:px-4 py-2 rounded-lg hover:bg-gray-100 focus-visible:outline-blue-600">
                        <div class="text-right hidden sm:block">
                            <div class="text-sm font-semibold text-gray-700 leading-tight">{{ auth()->user()->name }}
                            </div>
                            <div class="text-xs text-gray-500">{{ ucfirst(auth()->user()->role->nama_role) }}</div>
                        </div>
                        <div
                            class="w-9 h-9 bg-blue-500 rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0">
                            {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                        </div>
                        <x-icon name="chevron-down" class="w-4 h-4 text-gray-500 hidden sm:block"
                            aria-hidden="true" />
                    </button>

                    {{-- Dropdown --}}
                    <div id="admin-profile-dropdown" role="menu" x-show="profileOpen"
