...
@section('content')
    <div class="overflow-x-hidden">
        ...
        {{-- ── HERO BANNER ── --}}
        <div
            class="bg-gradient-to-br from-emerald-600 via-emerald-700 to-emerald-900 pt-10 pb-16 px-5 relative overflow-hidden">

            {{-- Decorative orbs --}}
            <div
                class="absolute top-0 right-0 w-72 h-72 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/4 blur-2xl pointer-events-none">
            </div>
            <div
                class="absolute bottom-0 left-0 w-56 h-56 bg-amber-400/10 rounded-full translate-y-1/2 -translate-x-1/4 blur-2xl pointer-events-none">
            </div>
        ...
        <div class="max-w-2xl mx-auto px-5 mt-4 pb-8 relative z-10 space-y-7">
            ...
            <div class="grid grid-cols-3 gap-3.5">
                @foreach ([['num' => $modules->count(), 'label' => 'Modul'], ['num' => $completedModules ?? 0, 'label' => 'Selesai'], ['num' => $overallProgress . '%', 'label' => 'Progress']] as $stat)
                    <div
                        class="bg-white rounded-2xl p-4 text-center border-2 border-gray-200 hover:-translate-y-1 hover:border-emerald-400 hover:bg-emerald-50/30 transition-all duration-200">
                        <p
                            class="text-3xl font-black bg-gradient-to-br from-emerald-600 to-emerald-800 bg-clip-text text-transparent leading-none">
                            {{ $stat['num'] }}
                        </p>
                        <p class="text-[11px] font-bold text-gray-400 uppercase tracking-widest mt-1.5">{{ $stat['label'] }}
                        </p>
                    </div>
                @endforeach
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-3.5">
                    @foreach ([['href' => route('siswa.materi.index'), 'emoji' => '📚', 'bg' => 'bg-emerald-50', 'name' => 'Belajar', 'sub' => 'Materi Iqra'], ['href' => route('siswa.kuis.index'), 'emoji' => '📝', 'bg' => 'bg-blue-50', 'name' => 'Kuis', 'sub' => 'Uji Kemampuan']] as $action)
                        <a href="{{ $action['href'] }}"
                            ...
                            <div
                                class="w-14 h-14 {{ $action['bg'] }} rounded-2xl flex items-center justify-center text-3xl mx-auto mb-3 group-hover:scale-110 group-hover:-rotate-6 transition-transform duration-300">
                                {{ $action['emoji'] }}
                            </div>
            ...
            <div>
                <div class="flex items-center justify-between mb-3.5">
                    ...
                    <a href="{{ route('siswa.materi.index') }}" aria-label="Lihat Semua Modul"
                        ...
                        Semua
                        <svg aria-hidden="true" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5"
                            viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3.5">
                    @foreach ($modules as $module)
                        ...
                        <a href="{{ route('siswa.materi.index', ['module' => $module->id]) }}"
                            aria-label="Mulai {{ $module->nama_modul }}"
                            ...
                            <p
                                class="text-4xl font-black bg-gradient-to-br from-emerald-600 to-emerald-800 bg-clip-text text-transparent leading-none mb-2">
                                {{ $arabicNum }}
                            </p>
                            ...
                            <div class="mt-auto pt-3.5">
                                ...
                                <p class="text-[11px] text-gray-400 font-bold mt-1.5 text-right">
                                    {{ $done }}/{{ $total }}</p>
                            </div>
                        </a>
                    @endforeach
