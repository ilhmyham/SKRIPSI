...
@push('styles')
    ...
    <style>
        /* Custom Plyr theme to match the app */
        :root {
            --plyr-color-main: #059669; /* Emerald 600 */
        }
        .plyr--video {
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 relative">
    ...
    <div class="flex justify-between items-center mb-6">
        @if($previousModule)
            <a href="{{ route('siswa.materi.index', ['module' => $previousModule->id]) }}" aria-label="Ke Modul Sebelumnya"
               class="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-lg border-2 border-gray-200 text-gray-700 hover:bg-gray-50 transition-all focus-visible:outline-emerald-600 focus-visible:outline-offset-2">
                <x-icon name="chevron-left" class="w-5 h-5" aria-hidden="true" />
        ...
        @if($nextModule)
            <a href="{{ route('siswa.materi.index', ['module' => $nextModule->id]) }}" aria-label="Ke Modul Berikutnya"
               class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition-all shadow-md hover:shadow-lg focus-visible:outline-emerald-700 focus-visible:outline-offset-2">
                <span class="text-sm font-medium hidden sm:inline">Berikutnya</span>
                <x-icon name="chevron-right" class="w-5 h-5" aria-hidden="true" />
            </a>
        @else
            <div class="w-24"></div>
        @endif
    ...
    <div x-data="{ open: ! localStorage.getItem('tipDismissed') }" class="mb-6">
        ...
        >
            ...
            <button
                ...
                class="flex-shrink-0 text-blue-400 hover:text-blue-600 transition focus-visible:outline-blue-600 rounded p-1"
                aria-label="Tutup panduan cara belajar"
            >
    ...
    <div class="mb-12">
                        ...
                        @foreach($kategoriList as $kategori)
                            @if(isset($materiData[$kategori]) && count($materiData[$kategori]) > 0)
                                <button
                                    ...
                                    :aria-label="activeTab === '{{ $kategori }}' ? 'Tab Kategori {{ $kategoriInfo[$kategori]['label'] }} Aktif' : 'Tab Kategori {{ $kategoriInfo[$kategori]['label'] }}'"
                                    :class="activeTab === '{{ $kategori }}' ? 'bg-emerald-500 text-white shadow-md focus-visible:outline-emerald-600' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 focus-visible:outline-gray-400'"
                                    class="flex-1 min-w-[100px] px-6 py-3 rounded-xl font-bold text-sm uppercase tracking-wide transition-all duration-200 whitespace-nowrap focus-visible:outline-offset-2"
                                >
                            ...
                            <div x-show="activeTab === '{{ $kategori }}'"
                                ...
                                <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2.5 md:gap-4 mb-6">
                                    <template x-for="materi in getPaginatedItems('{{ $kategori }}')" :key="materi.id">
                                        <button
                                            ...
                                            :aria-label="(materi.is_completed ? 'Selesai: ' : 'Belajar: ') + materi.judul_materi"
                                            class="group relative bg-white border-2 border-gray-200 rounded-xl p-2.5 hover:border-emerald-400 hover:shadow-lg transition-all duration-300 flex flex-col items-center justify-center aspect-square text-center focus:outline-none focus-visible:border-emerald-500 focus-visible:ring-4 focus-visible:ring-emerald-200"
                                        >
                                            {{-- Completion Badge --}}
                                            <template x-if="materi.is_completed">
                                                <div class="absolute top-1.5 right-1.5 w-5 h-5 bg-emerald-500 rounded-full flex items-center justify-center shadow-md">
                                                    <svg aria-hidden="true" class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="flex items-center justify-center gap-2 flex-wrap mt-8" x-show="getTotalPages('{{ $kategori }}') > 1">
                                    <button
                                        ...
                                        class="px-4 py-2 bg-white border-2 border-gray-200 rounded-lg font-medium text-gray-700 transition-all focus-visible:outline-emerald-500">
                                        ← Previous
                                    </button>
                                    ...
                                    <template x-for="page in getTotalPages('{{ $kategori }}')" :key="page">
                                        <button
                                            ...
                                            :class="currentPage === page ? 'bg-emerald-500 text-white border-emerald-500 focus-visible:outline-emerald-700' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50 focus-visible:outline-emerald-500'"
                ...
                <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2.5 md:gap-4">
                    @foreach($materis as $materi)
                        <button
                            ...
                            aria-label="{{ $materi->progressBelajar?->first()?->status == 'selesai' ? 'Selesai: ' : 'Belajar: ' }}{{ $materi->judul_materi }}"
                            class="group relative bg-white border-2 border-gray-200 rounded-xl p-2.5 hover:border-emerald-400 hover:shadow-lg transition-all duration-300 flex flex-col items-center justify-center aspect-square text-center focus:outline-none focus:ring-4 focus-visible:border-emerald-500 focus-visible:ring-emerald-200"
                        >
        ...
        @else
            <div class="text-center py-20 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                <svg aria-hidden="true" class="w-16 h-16 mx-auto mb-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex flex-wrap gap-2">
            @foreach($modules as $module)
                <a href="{{ route('siswa.materi.index', ['module' => $module->id]) }}"
                   :aria-current="{{ $currentModule && $currentModule->id == $module->id ? "'page'" : 'null' }}"
                   class="px-4 py-2 rounded-lg text-sm font-medium transition-colors focus-visible:outline-emerald-500 focus-visible:outline-offset-2 {{ $currentModule && $currentModule->id == $module->id ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50' }}">
                    {{ $module->nama_modul }}
                </a>
            @endforeach
        </div>
        ...
        <div class="flex min-h-full items-center justify-center p-4 sm:p-0">
            <div class="relative w-full transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all my-4 sm:my-8 sm:w-full sm:max-w-4xl border border-gray-200">
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 flex justify-between items-center border-b border-gray-100">
                    ...
                    <button type="button" onclick="closeModal()" class="rounded-full p-1 hover:bg-gray-200 transition-colors focus:outline-none" aria-label="Tutup modal belajar">
                        <svg aria-hidden="true" class="h-6 w-6 text-gray-500" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2">
                    <form id="formSelesai" action="" method="POST">
                        ...
                        @method('POST')
                        <button type="submit"
                                class="inline-flex w-full justify-center rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-emerald-500 sm:ml-3 sm:w-auto transition-all active:scale-95 focus-visible:outline-emerald-700 focus-visible:outline-offset-2 border hover:border-emerald-600">
                            ✅ Tandai Selesai
                        </button>
                    </form>
                    <button type="button" onclick="closeModal()"
                            class="mt-3 inline-flex w-full justify-center rounded-lg bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto focus-visible:outline-gray-500 focus-visible:outline-offset-2 border hover:border-gray-300">
                        Tutup
                    </button>
                </div>
            </div>
...
@push('scripts')
    ...
    <script src="{{ asset('js/materi-player.js') }}?v={{ filemtime(public_path('js/materi-player.js')) }}" defer></script>
@endpush
