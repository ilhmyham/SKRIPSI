        ...
        <div class="max-w-2xl mx-auto relative z-10 flex flex-col items-center text-center gap-4">
            ...
            <form method="POST" action="{{ route('logout') }}">
                ...
                <button type="submit" aria-label="Keluar dari sesi"
                    ...
                    <svg aria-hidden="true" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden"
            ...
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                ...
                <button @click="editing = !editing; if(!editing){ password=''; passwordConfirm=''; }"
                        type="button" aria-label="Edit formulir profil"
                    ...
                    <svg aria-hidden="true" x-show="!editing" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                    </svg>
                    <svg aria-hidden="true" x-show="editing" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" style="display:none">
            ...
            <form method="POST" action="{{ route('siswa.profile.update') }}" enctype="multipart/form-data" class="px-5 py-4 space-y-4">
                ...
                <div x-show="editing" style="display:none" class="flex items-center gap-4 p-3 bg-emerald-50 rounded-xl border border-emerald-100">
                    <div class="w-16 h-16 rounded-full overflow-hidden bg-emerald-100 border-2 border-emerald-200 flex items-center justify-center shrink-0">
                        @if($user->avatar)
                            ...
                            @endif
                        @else
                            <svg aria-hidden="true" x-show="!photoPreview" class="w-8 h-8 text-emerald-400" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"/>
                            </svg>
                        @endif
                    ...
                    <div>
                        ...
                        <label for="photo"
                            ...
                            <svg aria-hidden="true" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                            </svg>
                            Pilih Foto
                        </label>
                        <p class="text-[11px] text-gray-400 mt-1" x-show="!photoFile && !photoError">JPG, PNG. Max 2MB</p>
                        ...
                        <p class="text-[11px] text-red-500 mt-1 font-semibold flex items-center gap-1" x-show="photoError" style="display:none">
                ...
                {{-- Nama --}}
                <div>
                    <label for="name" class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1.5">Nama Lengkap</label>
                ...
                {{-- Email --}}
                <div>
                    <label for="email" class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1.5">Email</label>
                ...
                <div x-show="editing" style="display:none" class="space-y-3">
                    <div class="flex items-center gap-2 px-3 py-2.5 bg-blue-50 rounded-xl border border-blue-100">
                        <svg aria-hidden="true" class="w-4 h-4 text-blue-500 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    {{-- New Password --}}
                    <div>
                        <label for="password" class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1.5">Password Baru (Opsional)</label>
                        <div class="relative">
                            ...
                            <button type="button" @click="showPassword = !showPassword" aria-label="Lihat Password"
                                ...
                                <svg aria-hidden="true" x-show="!showPassword" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                </svg>
                                <svg aria-hidden="true" x-show="showPassword" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="display:none">
                    ...
                    {{-- Confirm Password --}}
                    <div>
                        <label for="password_confirmation" class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1.5">Konfirmasi Password</label>
                        <div class="relative">
                            <input :type="showPasswordConfirm ? 'text' : 'password'" name="password_confirmation" id="password_confirmation"
                                   ...
                                   class="w-full px-4 py-3 pr-11 text-sm font-semibold rounded-xl border-2 border-emerald-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 bg-white outline-none transition-all duration-200">
                            <button type="button" @click="showPasswordConfirm = !showPasswordConfirm" aria-label="Lihat Konfirmasi Password"
                                    class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 focus-visible:outline-emerald-500">
                                <svg aria-hidden="true" x-show="!showPasswordConfirm" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                </svg>
                                <svg aria-hidden="true" x-show="showPasswordConfirm" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="display:none">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/>
                                </svg>
                            </button>
                        </div>
                    ...
                    @foreach($recentProgress as $progress)
                        <div class="flex items-center gap-3 p-3 bg-emerald-50/60 rounded-xl border border-emerald-100/60">
                            <div class="w-8 h-8 bg-emerald-100 rounded-full flex items-center justify-center shrink-0">
                                <svg aria-hidden="true" class="w-4 h-4 text-emerald-600" fill="currentColor" viewBox="0 0 20 20">
