...
class SiswaController extends Controller
{
    private function getStudentStats(int $userId): array
    {
        ...
        $completedMateri = ProgressBelajar::where('user_id', $userId)
    ...
    public function dashboard()
    {
        ...
        $modules = ModulIqra::withCount('materi')->with('materi:id,modul_iqra_id')->get();

        $completedIds = ProgressBelajar::where('user_id', $user->id)
            ->where('status', 'selesai')
            ->pluck('materi_id')
            ->flip();
    ...
    {
        $user = auth()->user();
        $modules = ModulIqra::orderBy('id')->get();

        $currentModuleId = $request->get('module', $modules->first()?->id);
        $currentModule = ModulIqra::find($currentModuleId);

        $previousModule = ModulIqra::where('id', '<', $currentModuleId)
            ->orderBy('id', 'desc')->first();
        $nextModule = ModulIqra::where('id', '>', $currentModuleId)
            ->orderBy('id')->first();

        $materis = Materi::with(['modulIqra', 'progressBelajar' => function ($q) use ($user) {
        ...
        $hasKategori = $materis->whereNotNull('kategori_materi')->isNotEmpty();
        ...
        if ($hasKategori) {            
            ...
            $allKategoriConfig = [
                'hijaiyah'        => ['label' => 'Hijaiyah',    'fullLabel' => 'HURUF HIJAIYAH',       'color' => 'emerald'],
            ...
            $kategoriList = array_values($existingKategori);
    ...
    {
        $kuisList = Kuis::with(['modulIqra', 'kuisPertanyaan'])->get();
    ...
    public function showKuis(Kuis $kuis)
    {
        ...
        $hasAnswered = KuisJawabanSiswa::where('kuis_id', $kuis->id)
    ...
    public function startKuis(Kuis $kuis)
    {
    ...
    public function kuisResults(Kuis $kuis)
    {
        ...
        $userAnswers = KuisJawabanSiswa::where('kuis_id', $kuis->id)
            ->where('user_id', auth()->id())
    ...
    public function profile()
    {
        ...
        $completedKuis = KuisJawabanSiswa::where('user_id', $user->id)
            ->distinct('kuis_id')
            ->count('kuis_id');
        ...
        $recentProgress = ProgressBelajar::where('user_id', $user->id)
            ->with('materi')
            ->latest('updated_at')
            ->limit(5)
            ->get();
